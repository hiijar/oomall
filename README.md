...
### 2019.12.13 更新日志 
- footService模块增加了添加足迹 内部接口
- goodsService模块增加了通过goodsName查询goods的内部接口
- goodsService模块增加了通过id查询商品是否在售给分享模块的内部接口
- 把整个支付和微信支付模块内部接口都重写了一遍 细化了很多参数，并且在payment模块新增了管理员的查看和删除功能

### 2019.12.14更新日志
- 为满足discount模块的需求和传递参数统一，payment模块向order模块提供的接口的入参类型改为Payment
